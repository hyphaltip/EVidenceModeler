...
fn evm_startframe_to_gff3_phase(frame: u8) -> u8 {
    match frame { 1 | 4 => 0, 2 | 5 => 2, 3 | 6 => 1, _ => 0 }
}
...
pub fn evm_output_to_gff3(
    ...
    for line in reader.lines() {
        ...
        if line.starts_with('#') {
            ...
            if line.contains("EVM") {
                let ev_type = if line.contains("ELIMINATED") { "EVM_elm" } else { "EVM" };
                models.entry(model_id).or_insert_with(|| Model { coords: Vec::new(), ev_type }).ev_type = ev_type;
            }
            continue;
        }
            ...
            if let (Ok(e5), Ok(e3), Ok(frame)) = (
                ...
                if cols[2] != "INTRON" {
                    models.entry(model_id)
                        .or_insert_with(|| Model { coords: Vec::new(), ev_type: "EVM" })
                        .coords.push((e5, e3, frame));
                }
            }
        }
    }
    ...
    for (mid, model) in &models {
        if model.coords.is_empty() { continue; }
...
fn emit_model(out: &mut dyn Write, contig: &str, model_id: u32, model: &Model) -> Result<()> {
    ...
    for (i, &(e5, e3, frame)) in ordered.iter().enumerate() {
        ...
        let exon_num = i + 1;
        writeln!(out, "{}\t{}\texon\t{}\t{}\t.\t{}\t.\tID={}.exon{};Parent={}",
